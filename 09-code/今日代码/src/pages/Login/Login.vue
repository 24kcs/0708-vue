        ...
        <form>
          <div :class="{on:loginWay}">
            <section class="login_message">
              ...
              <!--v-show="errors.has('phone')" class="help is-danger"-->
          ...
          <div :class="{on:!loginWay}">
            <section>
              ...
              <section class="login_verification">
                <input :type="isPwdShow?'text':'password'" maxlength="8" placeholder="密码" />
                <div class="switch_button" :class="isPwdShow?'on':'off'" @click="isPwdShow=!isPwdShow">
                  <div class="switch_circle" :class="{circle_show:isPwdShow}"></div>
                  <span class="switch_text">{{isPwdShow?'abc':'...'}}</span>
                </div>
              </section>
              <section class="login_message">
                <input type="text" maxlength="11" placeholder="验证码" />
                <img ref="im" class="get_verification" src="http://localhost:5000/captcha" alt="captcha" @click="sendCaptcha" />
              </section>
            </section>
          </div>
...
export default {
  ...
  data () {
    return {
      loginWay:true, // 默认是true, 手机方式登录
      phone:'', // 手机号码
      computedTime:0, // 倒计时
      isPwdShow:false, // 默认是false,是文本框
    }
  },
  computed: {
    isRightPhone(){
